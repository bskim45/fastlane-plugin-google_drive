...
describe Fastlane::Actions::GoogleDriveUploadAction do
  ...
  it 'raise an error if keyfile does not exists' do
    expect do
      Fastlane::FastFile.new.parse("lane :test do
        google_drive_upload(drive_keyfile: 'test.json')
  ...
  it 'raise an error if no folder id was given' do
    expect do
      Fastlane::FastFile.new.parse("lane :test do
        google_drive_upload(drive_keyfile: '#{@key_path}')
      end").runner.execute(:test)
  ...
  it 'raise an error if no upload file was given' do
    expect do
      Fastlane::FastFile.new.parse("lane :test do
        google_drive_upload(drive_keyfile: '#{@key_path}', folder_id: 'some_id', upload_files: [])
      end").runner.execute(:test)
  ...
  it "raise an error if upload file does not exist" do
    expect do
      Fastlane::FastFile.new.parse("lane :test do
        google_drive_upload(drive_keyfile: '#{@key_path}', folder_id: 'some_id', upload_files: ['nofile'])
      end").runner.execute(:test)
  ...
  it "raise an error if folder_id does not exist" do
    expect do
      Fastlane::FastFile.new.parse("lane :test do
        google_drive_upload(drive_keyfile: '#{@key_path}', folder_id: 'some_id', upload_files: ['#{@upload_file}'])
      end").runner.execute(:test)
  ...
  it "raise an error if file upload fails" do
    ...
    Fastlane::FastFile.new.parse("lane :test do
      google_drive_upload(drive_keyfile: '#{@key_path}', folder_id: '#{folder_id}', upload_files: ['#{@upload_file}'])
    end").runner.execute(:test)
